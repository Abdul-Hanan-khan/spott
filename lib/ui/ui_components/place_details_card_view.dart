...
class PlaceDetailsPostCardView extends StatefulWidget {
  final bool addNavigationToComments;
  final int index;
  Post post;

  PlaceDetailsPostCardView(
      this.index, {
  ...
  @override
  State<PlaceDetailsPostCardView> createState() => _PlaceDetailsPostCardViewState();
}

class _PlaceDetailsPostCardViewState extends State<PlaceDetailsPostCardView> {
      ...
      child: Card(
        ...
        child: Column(
          ...
          children: [
            ...
            _buildUserProfile(context,_post),
            const Divider(),
            _PostFooterView(
              widget.index,
  ...
  Widget _buildUserProfile(BuildContext context,Post _post) {
...
class _PostHeaderView extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return ListTile(
        onTap: post.place != null
          ...
          Navigator.of(context).push(
            MaterialPageRoute(
              builder: (context) => PlaceDetailScreen(post.place!),
            ),
          );
        }
            : null,
        ...
        leading: InkWell(
            onTap: () {
              print(post.placeId);
              if (post.place?.placeStoryyAvailable == true) {
                Navigator.push(
                    context,
                    MaterialPageRoute(
                      builder: (context) => UserViewStoriesScreen(
                          post.placeId, post.place!.name!, post.id),
                    ));
              }
            },
            child: Container(
                decoration: BoxDecoration(
                    border: post.place?.placeStoryyAvailable == true
                        ? post.seen == false
                        ? Border.all(
                      color: Color(0xff33cc66),
                      style: BorderStyle.solid,
                ...
                child: PlaceImageView(post.place?.images?.firstOrNull))),
        title: Text(post.place?.name.toString() ?? ''),
        subtitle: Text(
          post.place?.fullAddress ?? '',
        ...
        trailing: Container(
          ...
          child: (post.myPost != null && post.myPost == true)
              ? PopupMenuButton(
  ...
  void _onPopupMenuSelection(BuildContext context, PopMenuOption value) {
    switch (value) {
      case PopMenuOption.reportPost:
        context
            .read<PostCardViewCubit>()
            .reportPost(int.parse(post.id.toString()));
        break;
      case PopMenuOption.blockUser:
        context
            .read<PostCardViewCubit>()
            .blockUser(int.parse(post.id.toString()), post.user?.id);
        break;
  ...
  deletePost(BuildContext context) async {
    context
        .read<PostCardViewCubit>()
        .deletePost(int.parse(post.id.toString()), context)
        .then((value) {
      print("Deleted");
