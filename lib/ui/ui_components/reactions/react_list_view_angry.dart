...
class AngryListViewScreen extends StatefulWidget {
  final PostReactsModel reacts;
  const AngryListViewScreen({Key? key, required this.reacts}) : super(key: key);

  @override
  _AngryListViewScreenState createState() => _AngryListViewScreenState();
}

class _AngryListViewScreenState extends State<AngryListViewScreen> {
  @override
  Widget build(BuildContext context) {
    ...
    return ListView.builder(
        ...
        itemBuilder: (context, index) {
          if (widget.reacts.data![index].reactKey == 5) {
            return InkWell(
              onTap: () {
                Navigator.push(
                    context,
                    MaterialPageRoute(
                        builder: (context) =>
                            ViewProfileScreen(widget.reacts.data![index].user)));
              },
              child: Container(
